...
| `⛔️ 禁止访问` / `Reject` | Reject policy for hijacking and HTTPDNS blocking rules. |
...
| `rules/block/` | Hijacking and HTTPDNS blocking rules |
...
| `⛔️ 禁止访问` / `Reject` | 拦截策略，用于防劫持和 HTTPDNS 屏蔽规则。 |
